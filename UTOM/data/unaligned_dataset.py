...
class UnalignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if B_path.endswith('.tif') or B_path.endswith('.tiff'):
        ...
        if len(A_img1.shape)==3:
            ...
            # stack to be 3 channels
            A_img2 = np.tile(A_img2, (1, 1, 3))
        ...
        # randome crop to input size
        # crop = transforms.CenterCrop(self.opt.crop_size)
        resize = transforms.Resize((self.opt.load_size, self.opt.load_size))
        A = resize(A)
        B = resize(B)
